...
CREATE TABLE municipio ( `id` INT NOT NULL AUTO_INCREMENT , `codigo` VARCHAR(2) NOT NULL , `nombre` VARCHAR(50) NOT NULL , `codigodepto` INT NOT NULL, 
							PRIMARY KEY (`id`),
							constraint codigodepto foreign key (`codigodepto`) references `departamento` (`codigo`) on delete restrict on update restrict)
...
CREATE TABLE desarrollador ( `codigo` INT NOT NULL AUTO_INCREMENT , `nombres` VARCHAR(50) NOT NULL , `apellidos` VARCHAR(50) NOT NULL , 
							...
							constraint codigodepto foreign key (`codigodepto`) references `departamento` (`codigo`) on delete restrict on update restrict,
							constraint codigomuni foreign key (`codigomuni`) references `municipio` (`codigo`) on delete restrict on update restrict,
							constraint codigousuario foreign key (`codigousuario`) references `usuario` (`codigo`) on delete restrict on update restrict)
