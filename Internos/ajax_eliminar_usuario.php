<?php 
	...
	$user = mysqli_query($conn, "SELECT * FROM usuario WHERE usuario ='$user' OR email='$email'");
	...
	$dev = mysqli_query($conn, "SELECT * FROM desarrollador WHERE codigousuario=$codigo");
	...
	if (mysqli_num_rows($dev) > 0) {
		...
		$dev_delete = mysqli_query($conn, "DELETE FROM desarrollador WHERE codigousuario=$codigo");
	}

	// Realizando la consulta
	$user_con = mysqli_query($conn, "DELETE FROM usuario WHERE codigo=$codigo");
