<?php
	...
	if (!$_GET) {
		header('Location: portafolio_in.php?pagina=1');
	}

	if ($_GET['pagina']>$paginas) {
		header('Location: portafolio_in.php?pagina='.$paginas);
	}
	if ($_GET['pagina']<=0) {
		header('Location: portafolio_in.php?pagina=1');
	}
	...
	$sql_pros_pag = "SELECT * FROM proyecto LIMIT $iniciar, $pro_x_pagina";
							...
							<nav aria-label="Page navigation example">
								<ul class="pagination justify-content-center">
								    ...
								    <?php echo $_GET['pagina']<=1 ? 'disabled' : '' ?>">
								      	<a class="page-link" 
								      	href="portafolio_in.php?pagina=<?php echo $_GET['pagina']-1; ?>">
								      		Anterior
								      	</a>
								    </li>
								    ...
								    <?php for ($i=0; $i < $paginas; $i++):?>
								    	...
								    	<?php echo $_GET['pagina']==$i+1 ? 'active' : '' ?>">
								    		<a class="page-link" href="portafolio_in.php?pagina=<?php echo $i+1; ?>"> <?php echo $i+1; ?> </a>
								    	</li>
								    <?php endfor ?>

								    <li class="page-item
								    <?php echo $_GET['pagina']>=$paginas ? 'disabled' : '' ?>">
								      	<a class="page-link" href="portafolio_in.php?pagina=<?php echo $_GET['pagina']+1; ?>">
								      		Siguiente
								      	</a>
								    </li>
							  	</ul>
